...
export const getSimilarPosts = async (categories, slug) => {
  ...
  `;
  const result = await request(blogGraphqlAPI, query, { slug, categories });
...
export const getAdjacentPosts = async (createdAt, slug) => {
  ...
  const result = await request(blogGraphqlAPI, query, { slug, createdAt });
...
export const getCategoryPost = async (slug) => {
  const query = gql`
    query GetCategoryPost($slug: String!) {
      postsConnection(where: {categories_some: {slug: $slug}}, last:5 ,orderBy: createdAt_DESC) {
        edges {
          cursor
          node {
            author {
