...
const GameCard = ({ game , showDeficit, scoreOnly}) => {
  ...
  let hidden = scoreOnly ? "hidden" : '';
  return (
              <Link href={`/game/${game.slug}`} className='rounded-lg bg-white p-2 lg:m-2 text-indigo-900 text-xs lg:text-sm shadow-md'>
                              <span className={`whitespace-nowrap text-center block font-bold ${hidden}`}>{toHumanReadable(game.dateAndTime)}</span>
                              <span className={`whitespace-nowrap text-center block text-red-800 font-bold${hidden}`}>{toHumanReadableTime(game.dateAndTime)}</span>
                              ...
                              {showDeficit && <span className={`whitespace-nowrap text-center block ${hidden}`}>{diff} Points d'ecart</span>}
                              {!showDeficit && game.gameState == "Terminated" && <span className={`text-center block ${hidden}`}>Fin</span>}
                </Link>
  )
}
