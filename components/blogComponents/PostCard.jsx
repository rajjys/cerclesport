...
const PostCard = ({blogPost, index}) => {
  let firstPost = index == 0;
  let evenPost = (index % 2 == 1)
  if(firstPost)
  return (
      ...
      <Link href={`/post/${blogPost.slug}`}>
        <div className='group bg-white shadow-lg rounded-lg p-2 mb-4 w-full md:flex'> 
          ...
          <div className='md:flex-initial'>
            ...
            <p className='text-left text-sm text-gray-700 font-normal px-4 lg:px-6 mb-4'>{blogPost.excerpt.substring(0, 200) + "..."}</p>
            <div className='px-4 block flex text-center items-center justify-around mb-4 w-full  text-s'>
              <div className='flex items-start justify-start mb-2 lg:mb-0 w-full lg:w-auto mr-8'>
                ...
                />   
                <p className='inline align-middle text-xs text-gray-700 ml-2'>{blogPost.author.name}</p>
              </div>
              <div className='text-xs text-gray-700'>
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 inline mr-2 text-pink-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                    <span className="align-middle">{moment(blogPost.createdAt).format('MMM DD, YYYY')}</span>
              </div>
            </div>
          </div>
      </div>
       ...
       <div className='group relative bg-white rounded-t-lg  rounded-b-md shadow-lg  w-48 h-64 transition duration-300 cusor-pointer ease-in-out hover:scale-110'>
          ...
          <div className='absolute bottom-0 text-gray-700 py-4 px-2 border-t border-yellow-500 w-full text-xs'>
            <div className='flex justify-between'>
              <p className='inline text-left' >{blogPost.author.name}</p>
              <p className="inline text-right">{moment(blogPost.createdAt).format('MMM DD, YYYY')}</p>   
            </div>
          </div>
       </div>
      </Link>
